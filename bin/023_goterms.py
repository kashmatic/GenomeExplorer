...
def generate_association():
	...
	newpath = os.path.abspath(os.path.join(os.pardir, 'bin', 'php', 'data', reference_go + '.association'))
...
def generate_population():
	...
	result = execute(sql)
	newpath = os.path.abspath(os.path.join(os.pardir, 'bin', 'php', 'data', reference_go + '.population'))
	print(newpath)
	fh = open(newpath, "w")
	for row in result:
...
def save_in_settings(afile, pfile):
	newpath = os.path.abspath(os.path.join(os.pardir, 'bin', 'php', 'settings', 'goatools.php'))
...
$gopopulation='{}';
?>
		""".format(program_goatools, re.sub(r".*bin/php/", '', afile), re.sub(r".*bin/php/", '', pfile))) 
		
if __name__ == '__main__':
	load_gff_file()
	apath = generate_association()
	ppath = generate_population()
	save_in_settings(apath, ppath)