...
$cols = array('rnaseq_expression.gene_id as id', 'gff.chr', 'gff.start', 'gff.end');
if($available_description){
	array_push($cols, 'gene_descriptions.description');
}
if(sizeof($mysql) == 1){
	array_push($cols, $mysql[0]);
} else {
	$cols = array_merge($cols, $mysql);
}
