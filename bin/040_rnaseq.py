...
def update_rnaseq(expslist, rnaseq_combine_file):
	php_dir = os.path.abspath(os.path.join(os.pardir, 'bin', 'php'))
	print(php_dir)
	expfile = os.path.join(php_dir, 'profile', 'experiments.php')
	with open(expfile, "w") as out:
		for experiment in expslist:
			out.write("<label><input type='checkbox' id='list' value='"+ experiment +"' checked='checked' >"+ experiment +"</label>\n")
...
def rnaseqexample(filename, examplefile, report = 5):
	fh = open(filename, "r")
	#fh.write('\t' + '\t'.join(expslist) + '\n')
	examplelist = []
	for line in fh:
		line = line.rstrip()
		cols = line.split("\t")
		examplelist.append("{}".format(cols[0]))
		if len(examplelist) >= report:
			break
	fh.close()
	with open(examplefile, 'w') as fh:
		fh.write('\n'.join(examplelist))
...
if __name__ == '__main__':
	...
	for filename in files:
		...
		dest = os.path.abspath(os.path.join('php', 'data', filename))
	...
	rnaseq_combine_file = os.path.abspath(os.path.join('php', 'data', 'rnaseq.combine'))
	...
	rnaseq_combine_file = os.path.abspath(os.path.join('php', 'data', 'rnaseq.combine'))
	example_file = os.path.abspath(os.path.join('php', 'profile', 'display.example.genes'))
	rnaseqexample(rnaseq_combine_file, example_file)
	example_file = os.path.abspath(os.path.join('php', 'profile', 'search.example.genes'))
	rnaseqexample(rnaseq_combine_file, example_file, 1)
	example_file = os.path.abspath(os.path.join('php', 'profile', 'cluster.example.genes'))
	rnaseqexample(rnaseq_combine_file, example_file, 20)
	example_file = os.path.abspath(os.path.join('php', 'sequence', 'goterm.example.genes'))
	rnaseqexample(rnaseq_combine_file, example_file, 27)
